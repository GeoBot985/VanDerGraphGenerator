...
def test_uses_deterministic_fallback_when_llm_disabled() -> None:
    ...
    result = _orchestrator(None).create_or_update_plan(
        "Show transactions per week", state, use_llm=True
    )
...
def test_uses_deterministic_fallback_when_ollama_fails() -> None:
    result = _orchestrator(
        LlmMappingResult(
            raw_response="", parsed_json=None, draft=None, errors=["offline"]
        )
    ).create_or_update_plan("Show transactions per week", _app_state(), use_llm=True)
    assert result.mapping_method == "deterministic_fallback"
    assert result.used_fallback is True
...
def test_stores_mapping_method_correctly() -> None:
    ...
    result = _orchestrator(
        LlmMappingResult(
            raw_response="{}", parsed_json={}, draft=draft, used_repair=True
        )
    ).create_or_update_plan("Show total amount by region", _app_state(), use_llm=True)
