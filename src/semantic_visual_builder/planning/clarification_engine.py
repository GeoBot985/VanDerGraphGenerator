...
class ClarificationEngine:
    ...
    def detect_needed_clarification(
        ...
        categorical = [column.name for column in dataset_profile.columns if column.semantic_type == "categorical"]
        numeric = [column.name for column in dataset_profile.columns if column.semantic_type == "numeric"]
        datetimes = [column.name for column in dataset_profile.columns if column.semantic_type == "datetime"]
        ...
        def add_request(question: str, reason: str, field_name: str | None, options: list[ClarificationOption]) -> None:
            requests.append(
        ...
        if x_role and x_role.field and x_role.field not in fields and x_role.field != "row_count":
            add_request(
                question=f"I could not find a column named {x_role.field}. Which field should be used?",
        ...
        if y_role and y_role.field and y_role.field not in fields and y_role.field != "row_count":
            add_request(
                question=f"I could not find a column named {y_role.field}. Which field should be used for the measure axis?",
        ...
        if plan.visual_kind == "chart":
            if plan.chart_type in {"bar", "horizontal_bar", "pie"}:
                ...
                if y_role is None or not y_role.field:
                    if len(numeric) > 1:
                        add_request(
                            question="Which numeric field should be used as the measure?",
            ...
            if plan.chart_type == "line":
                if x_role is None or not x_role.field:
                    if len(datetimes) > 1:
                        add_request(
                            question="Which date field should be used for the time axis?",
                ...
                if y_role is None or not y_role.field:
                    if len(numeric) > 1:
                        add_request(
                            question="Which numeric field should be used for the line values?",
            ...
            if plan.chart_type == "scatter":
                if x_role is None or not x_role.field or x_role.field not in numeric:
                    if len(numeric) > 1:
                        add_request(
                            question="Which numeric field should be used for the x axis?",
                ...
                if y_role is None or not y_role.field or y_role.field not in numeric:
                    if len(numeric) > 1:
                        add_request(
                            question="Which numeric field should be used for the y axis?",
            ...
            if plan.chart_type == "pie" and x_role and x_role.field in fields:
                ...
                if category_profile.unique_count > 6:
                    add_request(
                        question=f"{x_role.field} has {category_profile.unique_count} categories. Continue with a pie chart?",
                        reason="Pie charts are hard to read with many categories.",
                        field_name=x_role.role,
                        options=[
                            ClarificationOption(label="Continue", value="yes"),
                            ClarificationOption(label="Choose another chart", value="no"),
                        ],
                    )
    ...
    def apply_answer(
        ...
        if clarification.field_name in {"x", "category", "time_or_order"}:
            ...
            set_role(updated, clarification.field_name, answer_text, aggregation=aggregation)
        elif clarification.field_name and clarification.field_name not in {"x", "y"}:
            set_role(updated, clarification.field_name, answer_text)

        if clarification.reason.lower().startswith("pie charts are hard to read") and lowered in {"yes", "continue"}:
    ...
    def _match_option(self, options: list[ClarificationOption], answer: str) -> ClarificationOption | None:
